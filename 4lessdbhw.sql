DROP DATABASE IF EXISTS 4lessdbhw;
CREATE DATABASE 4lessdbhw;
USE 4lessdbhw;
...
CREATE TABLE answer SELECT media.id, media.user_id, profiles.birthday FROM media
LEFT JOIN profiles ON media.user_id=profiles.user_id;
SELECT * FROM answer;

CREATE TABLE answer1 SELECT * FROM answer
WHERE birthday > '2011-06-12';
SELECT * FROM answer1;

SELECT answer1.*, likes.user_id FROM answer1
LEFT JOIN likes ON answer1.id=likes.media_id;

DROP TABLE IF EXISTS answer;
CREATE TABLE answer SELECT answer1.*, likes.user_id AS E FROM answer1
LEFT JOIN likes ON answer1.id=likes.media_id;
DROP TABLE IF EXISTS answer1;
SELECT COUNT(E) FROM answer;

-- 2. Определить кто больше поставил лайков (всего): мужчины или женщины.

SELECT COUNT(profiles.gender), profiles.gender FROM likes
...
SELECT * FROM 
(SELECT m.from_user_id, p.user_id FROM messages m
LEFT JOIN profiles p ON m.from_user_id=p.user_id

UNION

SELECT m.from_user_id, p.user_id FROM messages m
RIGHT JOIN profiles p ON m.from_user_id=p.user_id

WHERE m.from_user_id IS NULL) AS R
